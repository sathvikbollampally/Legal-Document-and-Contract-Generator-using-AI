...
def string_to_word_doc(text, filename, title=None):
    ...
    for line in lines:
        ...
        elif stripped.lower().startswith("this ") and "agreement" in stripped.lower():
            p = doc.add_paragraph()
            run = p.add_run(stripped)
            run.bold = True
            run.font.size = Pt(13)
        ...
        elif "this document is a template and should be reviewed by a legal professional" in stripped.lower():
            p = doc.add_paragraph()
            run = p.add_run(stripped)
            run.bold = True
            run.font.size = Pt(12)
